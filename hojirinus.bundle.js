...
(function () {
  ...
  __def('config', function () {
    ...
    const CFG = {
      ...
      coinGodSet: 30000,      // 四神コンプ時の一時金。強化が一段飛ぶくらいの額にする
    
      // 図鑑ボーナス：集めた種類数がそのままコイン倍率になる。
      // 図鑑は初回ボーナスを取ったら経済的な役目が終わっていて、39種まで広げたのに
      // もったいなかった。集める行為がそのまま稼ぎに効くようにして、
      // 「ほじる→集める」と「飛ばす→強化」の両輪を噛み合わせる。
      // 最大 1 + 0.6 + 0.2 + 0.2 = 2.0倍。
      dexMulMax:    0.6,      // 通常枠をコンプで +60%
      dexMulGods:   0.2,      // 四神コンプで +20%
      dexMulSecret: 0.1,      // シークレット1体につき +10%
      ...
      //
      // 1.012 だと、100回ぶんのコストが常に3.30倍で増えるのに対し、収入の伸びは
      // 3.24 → 2.42 → 2.04 → 1.82 → 1.36倍 と減衰する（飛距離は v² で伸びるが、
      // effBonus が上限で頭打ちになると多項式的にしか伸びない）。指数 vs 多項式なので
      // 必ず発散し、100回強化するのに必要な投数が 163 → 226 → 364 → 659 と膨らんでいた。
      // 1.008 にすると最後まで80〜130投でほぼ一定になる。
      // ここはコイン倍率をいくら足しても直らない（線形の倍率では指数に勝てない）ので、
      // 曲線そのものを寝かせるのが正解。
  ...
  __def('state', function () {
    ...
    var { BOOGERS, BY_ID, NORMAL, SECRETS, GODS } = __req('boogers');
    ...
    const seenGods    = () => GODS.filter(b => hasSeen(b.id)).length;
    const godSetDone  = () => seenGods() === GODS.length;
    ...
    return { addCoins, buy, canBuy, consume, costOf, dexMul, digNeed, distanceOf, effBonus, gain, godSetDone, hasSeen, launchV, load, milestone, pickPower, playerPower, powerRatio, roll, S, save, seenGods, seenNormal, seenSecrets, session, simDist, speedMul, stockOf, totalStock, updateRecord, upLevel, upRate, wipe };
  });
  ...
  __def('pick', function () {
    ...
    var { clamp, lerp, easeOut, easeOutBack, TAU, fmt } = __req('util');
    ...
    var { RARITY_COLOR, RARITY_STARS, RARITY_LABEL, BY_ID } = __req('boogers');
  ...
  __def('fly', function () {
    ...
    var { S, stockOf, consume, addCoins, launchV, distanceOf, powerRatio, updateRecord, milestone, dexMul, } = __req('state');
  ...
  __def('shop', function () {
    ...
    var { S, costOf, canBuy, buy, upRate, upLevel, milestone, simDist, powerRatio, digNeed, dexMul, } = __req('state');
  ...
  __def('collection', function () {
    ...
    var { S, hasSeen, stockOf, seenNormal, seenSecrets, dexMul } = __req('state');
