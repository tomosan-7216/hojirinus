...
(function () {
  ...
  __def('config', function () {
    ...
    const RARITY = ['N', 'R', 'SR', 'UR', 'SECRET'];
    
    const CFG = {
    
      // ── ほじり ───────────────────────────────
      // グリグリしてもしなくても同じ速度。企画の指定どおり、テンポを優先する。
      gaugeTime: 2.5,
  ...
  __def('boogers', function () {
    ...
    const RARITY_STARS = { N: 1, R: 2, SR: 3, UR: 4, SECRET: 5 };
    
    return { BOOGERS, BY_ID, NORMAL, RARITY_COLOR, RARITY_STARS, SECRETS };
  });
  ...
  __def('state', function () {
    ...
    return { addCoins, buy, canBuy, consume, costOf, distanceOf, effBonus, gain, hasSeen, launchV, load, milestone, pickPower, playerPower, powerRatio, roll, S, save, seenNormal, seenSecrets, session, simDist, speedMul, stockOf, totalStock, updateRecord, upLevel, upRate, wipe };
  });
  ...
  __def('pick', function () {
    ...
    var { roll, gain, milestone, S } = __req('state');
    var { RARITY_COLOR, RARITY_STARS } = __req('boogers');
  ...
  __def('fly', function () {
    ...
    var { BOOGERS, RARITY_COLOR } = __req('boogers');
  ...
  __def('shop', function () {
    ...
    var { S, costOf, canBuy, buy, upRate, upLevel, milestone, simDist, powerRatio, } = __req('state');
  ...
  __def('collection', function () {
    ...
    var { BOOGERS, NORMAL, SECRETS, RARITY_COLOR } = __req('boogers');
